...
digits_kknn_test <- kknn(formula = as.factor(V65) ~ . , train = digits_train, test = digits_test, k = 30, kernel = "rectangular")
kknn_fit_test <- fitted(digits_kknn)
conf_mat_test <- table(obs = digits_test$V65, pred = kknn_fit_test)
conf_mat_test
...
shipunov::Misclass(fit, digits_train$V65)
#For misclassification rte : do it manually (no shipunov)
```
...
for(i in 1:30){
  #Fitting the classifier
  digits_kknn <- kknn(formula = as.factor(V65) ~ . , train = digits_train, test = digits_test, k = i, kernel = "rectangular")

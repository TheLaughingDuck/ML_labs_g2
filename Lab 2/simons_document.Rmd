...
train_noresponse <- train[,-which(names(test) == "ViolentCrimesPerPop")]
y_true <- train[,which(names(test) == "ViolentCrimesPerPop")]

# cost function
# data should NOT contain the response variable
# No intercept is included in the underlying model
cost_linreg <- function(theta){
  y_pred <- as.matrix(train_noresponse) %*% theta
  
  cost <- mean((y_pred - y_true)^2)
}
...
test_errors <- c()
for(i in 1:1){
  optim(theta, method="BFGS", fn = cost_linreg)

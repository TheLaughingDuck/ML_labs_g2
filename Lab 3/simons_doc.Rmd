...
We calculate this limit for the trained Neural Network from Assignment 3.1 to be about `r round(limit,2)`, and include the limit as a gold line in Figure 3.3.

## Assignment 3.5
Now we are interested in training a Neural Network to learn the arcsin function, which is the inverse of the sine function. To do this, we first generate $500$ observations in the interval $[0, 10]$, and then apply the sine function to each observation. We then train a Neural Network with the same structure as previously, but with the observations as response variable, and the sine function applied to the observations as the explanatory variable. In Figure 3.4 we plot the training observations with a green line, and the predictions (on the training data) as a red line. Note that the \textit{predicted} $\hat{x}$ are indicated on the horizontal axis, and \textit{explanatory} $\sin(x)$ are indicated on the vertical axis, in order to facilitate easy comparison with previous figures.
...
# Plot the result
plot(x = df_sindata$predicted_x,
     y = df_sindata$sin_x,
     col="red", type="l", cex=2, xlim = c(0, 10),
     main = "Fig 3.4a: Neural Network trained to fit the arcsine function.",
     xlab = "?",
     ylab = "?")
points(df_sindata %>% arrange(x), col="darkgreen", type = "l", lwd=1.5)
#points(arrange(mydata_0_to_50, Var), type="l", col="blue", lwd=2)
...
plot(x = df_sindata$sin_x,
     ...
     col="red", type="l", cex=2, xlim = c(-1, 1), ylim = c(0, 10),
     main = "Fig 3.4b: ",
     xlab = "?",
     ylab = "?")

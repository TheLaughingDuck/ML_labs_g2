...
## Analysis
When too few clusters are specified, the one or two clusters will attempt to approach the mean of the true distributions. If there is only one cluster, it will be centered around 0.5 for all dimensions, and only clusters will look similar to the two true clusters whose values go up and down but are centered around 0.5. When too many clusters are specified, the same thing will occur, but there may be redundancy. For the case of four clusters, The central $\mu=0.5$ for all D cluster is (roughly) replaced by two clusters centering around these values. In the case of five clusters, we observe that some clusters pick up the exact same mean values in some dimensions. There is a lot of overlapping here, because the clusters are picking up points in the same regions that should rather only be one cluster.
